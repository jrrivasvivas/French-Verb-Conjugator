...
class Conjugator:
    ...
    def generate_random_verb(selected_tenses):
        # Filtrar los tiempos verbales según los seleccionados por el usuario
        verb = random.choice(list(Conjugator.verb_dict.keys()))

        # Solo considerar los tiempos seleccionados por el usuario
        available_tenses = [tense for tense in Conjugator.verb_dict[verb].keys() if tense in selected_tenses]

        # Si no hay tiempos disponibles, usar un tiempo por defecto
        if not available_tenses:
            available_tenses = list(Conjugator.verb_dict[verb].keys())
